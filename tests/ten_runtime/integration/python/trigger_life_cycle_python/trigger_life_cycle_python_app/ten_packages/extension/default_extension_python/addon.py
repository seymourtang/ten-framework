...
from .extension import MainExtension, BizExtension
...
class DefaultExtensionAddon(Addon):
    def on_create_instance(
        ...
        if name == "main":
            ten_env.on_create_instance_done(MainExtension(name), context)
        elif name == "biz":
            ten_env.on_create_instance_done(BizExtension(name), context)
        else:
            assert False
