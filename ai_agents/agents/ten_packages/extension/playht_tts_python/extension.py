...
class PlayHTTTSExtension(AsyncTTS2BaseExtension):
    def __init__(self, name: str) -> None:
        super().__init__(name)
        self.config: PlayHTTTSConfig | None = None
        self.client: PlayHTTTS | None = None
    ...
    async def on_init(self, ten_env: AsyncTenEnv) -> None:
        ...
        try:
            self.config = PlayHTTTSConfig.model_validate_json(config_json)
            ten_env.log_info(
            ...
            self.client = PlayHTTTS(self.config.params)

            # pre connect and keep alive
            async for _chunk in self.client.synthesize_with_retry("P"):
                pass
                ...
                if not received_first_chunk:
                    ...
                    if is_new_request:
                        ...
                        await self.send_tts_ttfb_metrics(
                            ...
                            extra_metadata={
                                "voice_engine": self.config.params.voice_engine,
                                "voice": (
                                    self.config.params.voice
                                    if hasattr(self.config.params, "voice")
                                    else ""
                                ),
                            },
                        )
