...
static void ten_env_tester_proxy_notify_return_result(
  ...
  if (ctx->js_cb) {
    ...
    rc = ten_env_tester_return_result(ten_env_tester, ctx->c_cmd_result,
                                      proxy_return_result_error_callback, ctx,
                                      &err);
    if (!rc) {
    ...
    rc = ten_env_tester_return_result(ten_env_tester, ctx->c_cmd_result, NULL,
                                      NULL, &err);
    ten_env_tester_notify_return_result_ctx_destroy(ctx);
  }
...
napi_value ten_nodejs_ten_env_tester_return_result(napi_env env,
  ...
  if (callback_type != napi_undefined) {
    ...
    cb_tsfn = ten_nodejs_tsfn_create(
        env, "[TSFN] TenEnvTester::return_result callback", args[2],
        tsfn_proxy_return_result_callback);
